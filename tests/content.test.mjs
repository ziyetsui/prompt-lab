...
function promptFixture(overrides = {}) {
  return {
    ...
    publication: { publishedAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z', sourceRevision: REVISION },
...
async function fixtureRepository({ omitModelTaxonomy = false, prompt = promptFixture() } = {}) {
  ...
  for (const axis of omitModelTaxonomy ? ['content-type'] : ['content-type', 'model']) {
    const taxonomy = taxonomyFixture(axis)
...
test('temporary canonical draft Prompt and taxonomies pass the real schemas', async () => {
